...
# # --- ENGINE B: SULEIMAN INSTITUTIONAL ANALYTICS & FLOW ---
...
#         if abs(deviation) > 2.4 and not is_sideways: subpositions = 5  # Scale-in: Adds 4 high-conviction trades
#     elif deviation > 1.8:
...
#      with m1: st.markdown(f'<div class="metric-glow-box"><div class="metric-glow-label">Suleiman Clock HUD</div><div class="metric-glow-val" style="color:{timer_color}; font-size:{timer_font_size+2}px;">⏳ {res["candle_time_remaining"]}</div></div>', unsafe_allow_html=True)
...
#      # Suleiman HUD Text Injector
#      if timer_display_enabled:
#          text_y_position = res["entry"] + 0.5 if timer_position == "Above Current Price" else res["entry"] - 0.5
#          fig.add_trace(go.Scatter(
#              x=[market_df["time"].iloc[-1]], y=[text_y_position], mode="text",
#              text=[f"⏱️ {res['candle_time_remaining']}"], textposition="top center",
#              font=dict(color=timer_color, size=timer_font_size, family="JetBrains Mono"), showlegend=False
#          ))
...
# • Core Intent State: <b>{shared_res['direction']}</b>
# • Risk Lots Allocated: <code>{shared_res['calculated_lot']} Lots</code>
# • Structural Execution Entry Anchor: {shared_res['entry']}
# • Automated Stop Target Matrix: {shared_res['sl']}
# • Target Take Profit Layer: <code>{shared_res['tp']}</code>

# 🌐 <i>Sync Frame Server Timestamp: {shared_res['timestamp']} UTC</i>"""
                
#                  status_ok, err = send_telegram(payload_text)
#                  if status_ok: 
#                      st.toast("Outbound telemetry channel packets successfully transmitted!", icon="🚀")
#                  else: 
#                      st.error(f"Transmission Channel Refused: {err}")
...
def render_live_dashboard_layer(tf):
     ...
     # Patched HUD Text Injector (Explicit type-casting to satisfy Python 3.14/Plotly keyword logic)
     if timer_display_enabled:
         try:
